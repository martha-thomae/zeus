...
That is what you want when LMX is what you were after — e.g., when comparing two models' raw output, feeding a script that reads tokens, or measuring a model against gold LMX by hand. It also skips the decoder entirely, so even a prediction that would cause MusicXML conversion to choke still comes through; see [below](#when-a-staff-cannot-be-read).
